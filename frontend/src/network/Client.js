    ...
    connect() {
        ...
        this.ws.onmessage = (event) => {
            try {
                ...
                console.error('websocket: error decoding server message:', err);
            }
        };

        this.ws.onerror = (error) => {
            console.error('websocket: onerror called:', error);
    ...
    send(messageObj) {
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
            try {
                ...
                console.error('websoscket: error encoding or sending message:', err);
            }
        } else {
